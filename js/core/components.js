...
(function() {
    ...
    return `
      <nav class="navbar navbar-expand-lg sticky-top">
        <div class="container navbar-finora">
          ...
          <div class="navbar-actions" data-nav-auth${minimal ? " data-nav-minimal" : ""}></div>
        </div>
      </nav>`;
  }

  function compactFooterMarkup() {
    return `
  ...
  class FinoraHeader extends HTMLElement {
    connectedCallback() {
      this.innerHTML = headerMarkup(this.hasAttribute("minimal"));
    }
  }

  class FinoraFooter extends HTMLElement {
    connectedCallback() {
      this.innerHTML = this.getAttribute("variant") === "landing"
        ? landingFooterMarkup()
        : compactFooterMarkup();
    }
  }
